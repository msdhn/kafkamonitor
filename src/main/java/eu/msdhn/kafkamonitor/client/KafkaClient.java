    ...
    val allTopicPartitions = asJavaCollection(this.zkClient.getAllPartitions()).stream()
        .map(eachPartition -> {
          ...
          kafkaTopicPartition.setReplicas(
              asJavaCollection(this.zkClient.getReplicasForPartition(eachPartition)).stream()
                  .map(eachReplica -> {
        ...
        .collect(Collectors.toList());
    allTopicPartitions.sort((o1, o2) -> o1.getTopic().getName().compareTo(o2.getTopic().getName()));
